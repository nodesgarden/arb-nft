// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import {console2} from "forge-std/Script.sol";
import {NodeNFT} from "../src/NodeNFT.sol";
import {NodeNFTOperations} from "./utils/NodeNFTOperations.sol";

contract TransferNodeNFTBatch is NodeNFTOperations {
    function run() external {
        address contractAddress = vm.envAddress("NFT_CONTRACT");
        uint256 ownerPrivateKey = vm.envUint("OWNER_PRIVATE_KEY");
        string memory filePath = vm.envString("TRANSFER_BATCH_FILE");

        NodeNFT nft = NodeNFT(contractAddress);
        TransferBatch memory batch = _readTransferBatch(filePath);
        address owner = vm.addr(ownerPrivateKey);

        vm.startBroadcast(ownerPrivateKey);

        for (uint256 i = 0; i < batch.recipients.length; ++i) {
            nft.transferFrom(owner, batch.recipients[i], batch.tokenIds[i]);
        }

        vm.stopBroadcast();

        console2.log("Transferred tokens:", batch.recipients.length);
        console2.log("Contract:", contractAddress);
        console2.log("From:", owner);
        console2.log("Batch file:", filePath);
    }
}
